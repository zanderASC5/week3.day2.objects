...
const highlightHero = function(event){
    removeIds();
...
for(const hero_container of allHeroContainers){
    hero_container.addEventListener("click",highlightHero);
}
...
button.addEventListener("click",function(event){
    // remember to .preventDefault()
    event.preventDefault();
    // creating hero object (some defaults)
    const hero = {
        img:"sombra.png",
        story:"..this character has no backstory...yet!",
        abilities:[
            {
                name:"ability1",
    ...
    hero["difficulty"] = difficulty.value
    hero["saying"] = saying.value
    ...
    // adding new hero to "#gridDiv" (notice the array!)
    heroCreator.populateGrid(gridDiv,[hero])

    // getting last hero added to #gridDiv
    const allHeroContainers = document.getElementsByClassName("hero_container");
    const lastHero = allHeroContainers[allHeroContainers.length-1];

    lastHero.addEventListener("click",highlightHero);
});
